...
def create_secret(secretname, secretvalue):
    ...
    try:
        secret = client.api.create_secret(
            name=secretname, data=secretvalue.encode('utf-8'))
...
class Dksecrets(Resource):
    def post(self):
        ...
        Assuming that the service exists, this API creates a docker secret
        from the inputted name and value, then distribute the docker secret
        to the corresponding service
        Returns:
            [type] Json object -- [description] Successful message or error
            message
        '''
        ...
        try:
            ...
            service = client.services(filters={"name": service_name}).pop(
        ...
        # get service container specification
        container_spec = service['Spec']['TaskTemplate']['ContainerSpec']
        current_secrets = []
        try:
        ...
        secret_list = [docker.types.SecretReference(secret_id, secret_name)]
        container_spec['Secrets'] = current_secrets + \
            secret_list  # update list of secrets for the service
        # Create TaskTemplate from container specification
        task_tmpl = docker.types.TaskTemplate(container_spec)

        if DEBUG_MODE:
            print("Task template", task_tmpl.container_spec)

        # Update service with new secret list
        client.update_service(service=service_id, name=service_name,
                              version=service_version, task_template=task_tmpl)
...
class Dksecret(Resource):
    def delete(self, secret_name):
        ...
        [description]
        Remove secret from all container of a services. If there are no other
        services using the secret, delete the secret from docker swarm
        Arguments:
        ...
        try:
            ...
            if DEBUG_MODE:
                ...
                print('Secret name of 1st item: ',
                      current_secrets[0]['SecretName'])

            update_current_secrets = [
                x for x in current_secrets if x['SecretName'] != secret_name]
        ...
        if DEBUG_MODE:
            print("\nConfig of service after update:",
                  client.inspect_service(service=service_name))
    ...
    def get(self, secret_name):
        ...
        try:
            secret_id = client.api.secrets(
                filters={"name": secret_name})[0]['ID']
